...
function Commandments:addCmd(name,description,funciones,mod,...)
	local flags = {...};
	...
	table.insert(self,#self+1,{name,description,funciones,mod,fixedFlags});
end;
		...
		if msgObj.content:sub(1,#Commandments.Prefix) == Commandments.Prefix then
			...
			if Commandment ~= nil then
				...
				if Commandment[5].none then
					print('COMMANDMENTS | Commandment '..msgObj.content..' by '..msgObj.author.tag..' is valid, executing');
					Commandment[3](msgObj,Parameter);
					return true;
				else
					if Commandment[5].dmonly then
						if msgObj.guild then
							msgObj:reply('This command is DM only');
						else
							if Commandment[5].owneronly then
								if msgObj.author.id == Client.owner.id then
									Commandment[3](msgObj,Parameter);
									print('COMMANDMENTS | DM Commandment '..msgObj.content..' by bot owner is valid, executing');
									return true;
								else
									msgObj:reply('This command is bot owner only');
								end;
							else
								Commandment[3](msgObj,Parameter);
								print('COMMANDMENTS | DM Commandment '..msgObj.content..' by '..msgObj.author.tag..' is valid, executing');
								return true;
							end;
						end;
					elseif Commandment[5].serveronly then
						if not msgObj.guild then
							...
							if Commandment[5].owneronly then
								if msgObj.author.id == Client.owner.id then
									Commandment[3](msgObj,Parameter);
									print('COMMANDMENTS | Commandment '..msgObj.content..' by bot owner is valid, executing');
									return true;
								else
									msgObj:reply('This command is bot owner only');
								end;
...
return function(_Client)
	...
	while true do
		local mod = modIter();
		if mod ~= nil then
			print('COMMANDMENTS | Loading '..mod);
			table.insert(Commandments.Modules,require('./mods/'..mod)(Commandments,Client));
			print('COMMANDMENTS | Module '..Commandments.Modules[#Commandments.Modules]..' loaded');
		else
			break;
		end;
	end;
